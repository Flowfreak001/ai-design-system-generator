...
// How to authenticate a Wix Data call: either a connected app token (per-user
// OAuth) or, when omitted, the single-account env API key.
export type WixAuth = { token: string; siteId?: string | null };

function headers(auth?: WixAuth): Record<string, string> {
  if (auth) {
    const h: Record<string, string> = { "Content-Type": "application/json", Authorization: auth.token };
    if (auth.siteId) h["wix-site-id"] = auth.siteId;
    return h;
  }
...
async function post(url: string, body: unknown, auth?: WixAuth): Promise<{ ok: boolean; status: number; text: string }> {
  const res = await fetch(url, { method: "POST", headers: headers(auth), body: JSON.stringify(body), cache: "no-store" });
  const text = await res.text().catch(() => "");
  return { ok: res.ok, status: res.status, text };
}

/** Insert one item (create-only). Returns the new item id. */
export async function insertDataItem(collectionId: string, data: Record<string, unknown>, auth?: WixAuth): Promise<{ id: string }> {
  const r = await post(ITEMS_URL, { dataCollectionId: collectionId, dataItem: { data } }, auth);
...
export async function saveDataItem(collectionId: string, id: string, data: Record<string, unknown>, auth?: WixAuth): Promise<void> {
  const r = await post(SAVE_URL, { dataCollectionId: collectionId, dataItem: { id, data } }, auth);
...
export async function queryItemIds(collectionId: string, filter: Record<string, unknown>, auth?: WixAuth): Promise<string[]> {
  const r = await post(QUERY_URL, { dataCollectionId: collectionId, query: { filter, cursorPaging: { limit: 100 } } }, auth);
...
export async function removeDataItem(collectionId: string, id: string, auth?: WixAuth): Promise<void> {
  const r = await post(REMOVE_URL, { dataCollectionId: collectionId, dataItemId: id }, auth);
 ...
 */
export async function ensureCollection(id: string, displayName: string, fields: WixField[], auth?: WixAuth): Promise<void> {
