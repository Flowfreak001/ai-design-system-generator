...
export function createSectionTheme(styleGuide?: StyleGuideCanvas | null): SectionTheme {
  ...
  return {
    ...
    backgroundColor: byName(/bg|background|canvas|white|50/i) ?? DEFAULT_SECTION_THEME.backgroundColor,
    surfaceColor: byName(/surface|panel|100|200|muted/i) ?? DEFAULT_SECTION_THEME.surfaceColor,
    textColor: byName(/ink|text|black|900|800/i) ?? DEFAULT_SECTION_THEME.textColor,
    mutedTextColor: DEFAULT_SECTION_THEME.mutedTextColor,
    borderColor: byName(/border|line|200|300/i) ?? DEFAULT_SECTION_THEME.borderColor,
    radius: `${radiusPx}px`,
