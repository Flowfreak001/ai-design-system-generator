...
const Chip: React.FC<{ t: SectionTheme; children?: React.ReactNode }> = ({ t, children }) => {
  ...
  const box = <span className="grid h-11 w-11 place-items-center rounded-2xl text-[15px] font-semibold transition-transform group-hover:scale-105" style={{ background: tint(t, 16), color: t.accentColor, border: `1px solid ${tint(t, 30)}` }}>{children ?? <BlockIcon k={icon} />}</span>;
...
const Ph: React.FC<{ t: SectionTheme; className?: string; label?: string; rounded?: string }> = ({ t, className = "", label, rounded = "rounded-2xl" }) => {
  ...
  const content = url ? <img src={url} alt="" className="h-full w-full object-cover" /> : <span className="text-[11px] font-medium uppercase tracking-wider" style={{ color: t.mutedTextColor, opacity: 0.6 }}>{label ?? "image"}</span>;
  if (!onEdit) return <div className={cls} style={style}>{content}</div>;
  return (
    <div className={`${cls} cursor-pointer`} style={style} onPointerDown={(e) => e.stopPropagation()} onClick={(e) => { e.stopPropagation(); (e.currentTarget.querySelector("input") as HTMLInputElement | null)?.click(); }}>
      {content}
