...
export function ReferenceLibraryClient({ projectId, projectName, initialPatterns }: { projectId: string; projectName: string; initialPatterns: SectionPattern[] }) {
  ...
  const save = () => {
    ...
    start(async () => {
      ...
      setDraft(null); setFull(""); setThumb(""); setNotes("");
    });
  };
    ...
    <div className="mx-auto max-w-6xl p-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        ...
        <div className="card h-max p-4">
          <p className="text-[13px] font-semibold text-ink">Add a reference</p>
          <label className="mt-3 grid cursor-pointer place-items-center rounded-xl border border-dashed border-line py-6 text-center hover:border-accent">
            {thumb ? <img src={thumb} alt="reference" className="max-h-40 rounded-lg" /> : <span className="text-[12.5px] text-muted">Click to upload a section screenshot</span>}
        ...
        <div className="grid gap-6">
          ...
          <div>
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search patterns…" className="w-48 rounded-lg border border-line px-3 py-1.5 text-[13px]" />
            ...
            {visible.length === 0 ? (
              <div className="grid place-items-center rounded-2xl border border-dashed border-line py-14 text-[13px] text-muted">No saved patterns yet.</div>
            ) : (
